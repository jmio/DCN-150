...
static void GetNextRowToWrite(uint32 seqNum,
        uint32 * rowToWrPtr, uint32 * rowToRdPtr, cy_stc_eeprom_context_t * context);
...
static cy_en_em_eeprom_status_t CheckCrcAndCopy(uint32 startAddr,
    uint32 dstAddr, uint32 rowOffset, uint32 numBytes, cy_stc_eeprom_context_t * context);
...
cy_en_em_eeprom_status_t
        Cy_Em_EEPROM_Init(cy_stc_eeprom_config_t* config, cy_stc_eeprom_context_t * context)
{
...
* datasheet.
*
* In case in if redundant copy option is enabled the function may perform writes
...
*******************************************************************************/
cy_en_em_eeprom_status_t
    Cy_Em_EEPROM_Read(uint32 addr, void * eepromData, uint32 size, cy_stc_eeprom_context_t * context)
{
    cy_en_em_eeprom_status_t ret = CY_EM_EEPROM_BAD_PARAM;
    uint32 i;
    ...
    if((0u != size) && ((addr + size) <= (context->eepromSize)) && (NULL != eepromData))
    {
        ...
        uint32 seqNum = CY_GET_REG32(context->lastWrRowAddr);
        ...
        if(0u != seqNum)
        {
            ...
            */
            if(CY_EM_EEPROM_SUCCESS == ret)
            {
                numRowReads = context->numberOfRows - 1u;
                ...
                for(i = (seqNum - numRowReads); i <= seqNum; i++)
                {
                    ...
                    if (0u != startRowAddr)
                    {
                        ...
                        if(0u != CY_EM_EEPROM_IS_ADDRESES_CROSSING(startAddr, endAddr, addr, addr + size))
                        {
                            ...
                            eeData = ((uint32)eepromData) + (rdAddr - addr);
                            ...
                            */
                            if(0u != context->redundantCopy)
                            {
                                ret = CheckCrcAndCopy(startRowAddr, eeData,
                                                      CY_EM_EEPROM_HEADER_DATA_OFFSET, numBytesToRead, context);
                            ...
                            {
                                (void)memcpy((void *)(eeData),
                                             (void *)(startRowAddr + CY_EM_EEPROM_HEADER_DATA_OFFSET),
                                             numBytesToRead);
                            }
                        }
                    }
        ...
        {
            /* No records in the EEPROM */
            ret = CY_EM_EEPROM_BAD_DATA;
        }
    }
...
*******************************************************************************/
cy_en_em_eeprom_status_t
    Cy_Em_EEPROM_Write(uint32 addr, void * eepromData, uint32 size, cy_stc_eeprom_context_t * context)
{
    cy_en_em_eeprom_status_t ret = CY_EM_EEPROM_BAD_PARAM;
    uint32 i;
    uint32 wrCnt;
    ...
    if((0u != size) && ((addr + size) <= (context->eepromSize)) && (NULL != eepromData))
    {
        ...
        for(wrCnt = 0u; wrCnt < numWrites; wrCnt++)
        {
            ...
            /* Get the sequence number of the last written row */
            uint32 seqNum = CY_GET_REG32(emEepromRowAddr);
...
cy_en_em_eeprom_status_t Cy_Em_EEPROM_Erase(cy_stc_eeprom_context_t * context)
{
    ...
    /* Get the sequence number of the last written row */
    seqNum = CY_GET_REG32(emEepromRowAddr);
...
{
    return(CY_GET_REG32(context->lastWrRowAddr));
}
...
static void FindLastWrittenRow(uint32 * lastWrRowPtr, cy_stc_eeprom_context_t * context)
{
    ...
    *lastWrRowPtr = emEepromAddr;
    ...
    {
        seqNum = CY_GET_REG32(emEepromAddr);
...
static uint32 GetRowAddrBySeqNum(uint32 seqNum, cy_stc_eeprom_context_t * context)
{
    ...
    while(CY_GET_REG32(emEepromAddr) != seqNum)
    {
        /* Switch to the next row */
        emEepromAddr = emEepromAddr + CY_EM_EEPROM_FLASH_SIZEOF_ROW;
...
*
*******************************************************************************/
static cy_en_em_eeprom_status_t WriteRow(uint32 rowAddr, uint32 *rowData,
...
*
*******************************************************************************/
static cy_en_em_eeprom_status_t EraseRow(uint32 rowAddr, uint32 ramBuffAddr,
    cy_stc_eeprom_context_t * context)
{
    ...
    ramBuffAddr = ramBuffAddr; /* To avoid compiler warning */
...
*
*******************************************************************************/
static uint32 GetAddresses(uint32 *startAddr, uint32 *endAddr, uint32 *offset, uint32 rowNum,
